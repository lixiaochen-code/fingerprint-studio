...
| 路由 + 全局编排 | `src/App.tsx` |
| 单页 / 路由组件 | `src/views/`（按 view 拆分） |
| 应用框架级组件（header、shell） | `src/components/` 顶层 |
| 业务功能组件（弹窗、表单、专项视图） | `src/components/<name>/index.tsx` 或 `src/views/<view>/components/<name>/index.tsx` |
...
- 各 view 的展示组件放 `src/views/<name>/index.tsx`，由 `App.tsx` 用 `<KeepAlive>` 包一层（Monaco / 长连接需要保留实例时）
...
- 同一 view 的多个子区域优先抽 `src/views/<view>/components/<sub>/index.tsx`，不要全部塞进一个 `View.tsx`
...
- React 业务组件：`kebab-case/index.tsx`（如 `profile-form-dialog/index.tsx`）
- 路由组件文件：`src/views/<view>/index.tsx`，组件名 `PascalCase`
...
- **脚本子系统**：完整规格见 `specs/baseline/desktop/scripts/spec.md`。历史 handoff 记录在 `specs/archive/_cross/2026-05-legacy-handoffs/`
- **代理子系统（ProxyStore + 白名单 + 测试）**：设计与迁移逻辑见 `electron/proxies/`；UI 入口 `src/views/proxies/`、`src/components/proxy-form-dialog/`、`src/components/proxy-select-field/`。baseline 见 `specs/baseline/desktop/proxies/spec.md`（当前仍有 TBD 段，下一次相关 change 应补齐 Current Capabilities）。
- **内核子系统**：启动参数与内核选择在 `electron/kernel.ts`；baseline 见 `specs/baseline/desktop/kernel/spec.md`（当前仍有 TBD 段）。
- **profiles 子系统**：持久化在 `electron/store.ts`；UI 入口 `src/views/profiles/`。baseline 见 `specs/baseline/desktop/profiles/spec.md`（当前仍有 TBD 段）。
- **反检测三轨**：`specs/baseline/desktop/stealth/spec.md`；历史 handoff 在 `specs/archive/_cross/2026-05-legacy-handoffs/`

> 历史 handoff 保留决策背景，但不作当前状态真源。读旧交接包前先看 `specs/archive/_cross/2026-05-legacy-handoffs/handoffs/README.md`。
