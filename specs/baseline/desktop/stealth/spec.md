...
模块组织:`electron/stealth/`,按 patch 拆分。每个 patch 文件 export 一段浏览器侧 JS 字符串,由 `electron/stealth/index.ts` 的 `buildStealthInjectScript(toggles)` 按 toggle 拼成最终 inject.js。
